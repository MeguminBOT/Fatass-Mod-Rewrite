function onCreate()
	precacheImage('bulletnote')
	precacheSound('jebus-bullet')
	
	--Iterate over all notes
	for i = 0, getProperty('unspawnNotes.length')-1 do
		if getPropertyFromGroup('unspawnNotes', i, 'noteType') == 'ubershoot' then --Check if the note on the chart is a Bullet Note
...
function goodNoteHit(id, direction, noteType, isSustainNote)
	if noteType == 'ubershoot' then
		playSound('jebus-bullet', 0.15);
		characterPlayAnim('dad', 'ubershoot', true);
