...
local msDiffTimeLimit = 500  -- how many ms after hitting a note should the ms rating disappear
...
function onCreate()
    ...
    makeLuaText('msTxt', ' ', 0, 575, 400)
    setTextSize('msTxt', 26)
    setTextBorder('msTxt', 2, '000000')
    setTextFont('msTxt', 'rubik.ttf')
    addLuaText('msTxt')
    end
...
function showMsDiffOnScreen(diff)  -- remove everything in this function if you don't want ms timings.
    removeLuaText('msTxt', false)
    local msDiffStr = string.format("%.2fms", -diff)
